export const metadata = {
    ...
    thumbnailPath: "/images/thumbnails/antidebug.webp",
    animationPath: "/animations/antidebug/data.lottie",
    animationBackground: "/animations/antidebug/background.webp",
...
CTO of [Haxo Games Inc](/project/haxo-games). (a now defunct company). You can find it on my Github [here](https://github.com/axxo1337/AntiDebug).

<GithubRepo link="https://github.com/axxo1337/AntiDebug" />

### Design

For the CLI, the code depends on the library [FTXUI](https://github.com/ArthurSonzogni/FTXUI). The rendering logic runs on a thread independent of the detection logic. 
Synchronization is done using `std::atomic` and a mutex here and there. Nothing too fancy. I don't intend to work on this anymore since Haxo is gone, but I'm open to outside 
...
`CheckRemoteDebuggerPresent` is a Windows API function which calls `NtQueryInformationProcess` for the `ProcessDebugPort` value under the hood. If this value is 0
...
itself. `ProcessDebugPort` being in the `EPROCESS` structure it is immune to user-mode shenanigans.
...
`NtQuerySystemInformation` can be used to get `DebuggerInformation`. It's an undocumented structure made up of three booleans:
```c++
...
When closing an invalid handle an `EXCEPTION_INVALID_HANDLE (0xC0000008)` will be raised if a debugger is attached. This can then be caught by an `__except` block once the debugger passes
...
This method will raise the exception `DBG_PRINTEXCEPTION_C (0x40010006)`. If it is handled by the program itself then no debugger is attached. Otherwise one obviously is. It is usually raised
...
Bypassing is as simple as setting it to any value that does not match any of those flags (0 will work great). Also it seems these flags are only set when a program is launched with a debugger. When it is attached 
...
`CyclesPassed` uses MSVC intrinsics to check if the CPU cycles delta between function's prologue and epilogue was abnormally huge. If such a thing happened more than or equal 3 
times then it assumes that there was a breakpoint set inside it.

#### `IsWindowsFunctionBreakpointed`

`IsWindowsFunctionBreakpointed` scans for the first 1 or 2 bytes of WinAPI functions (mostly those used for anti-debugging) for software breakpoint INT3 (0xCC / 0xCD03) 
and an undefined opcode which can be used to throw an exception (0x0F0B).