...
The format of memory we'll mostly care about today is random access memory (RAM). This media is volatile meaning the method it uses to read and write bits is temporary. When you 
cut the power to your machine all of its contents are lost. It's also the second fastest storage format in computers after in-processor memory, and that's where programs will be
...
The figure above provides an example of memory addressing. Each row consists of a byte (8 bits) and is indexed as such (row 1 is 0x0, row 2 0x1 and row 3 0x3). You may be 
...
I said I wouldn't delve into hexadecimal, but I'll at least tell you how it supports this many digits if we only know 9: It uses the alphabet. Here's all these possible digits: 1,
