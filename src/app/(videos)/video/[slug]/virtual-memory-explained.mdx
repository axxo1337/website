export const metadata = {
    ...
    thumbnailPath: "/images/thumbnails/virtual-memory-explained.webp",
...
arise: security and resource management. On one hand each application should be mostly isolated from one another.
...
Memory is not an easy thing to define as it exists in _many_ forms. Although all formats of memory share one common property:
...
The parentheses and the suffix 2 are to let us know that the number is specifically in base 2 (binary) and not the decimal number 1011. So how do we convert this back to base 10?
...
![Binary number powers](/images/documents/virtual-memory-explained/binary_powers.webp)
...
![Binary results](/images/documents/virtual-memory-explained/binary_results.webp)
...
![Memory scale](/images/documents/virtual-memory-explained/memory_scale.webp)
...
![Example addressing](/images/documents/virtual-memory-explained/example_addressing.webp)
...
![Example scenario 1](/images/documents/virtual-memory-explained/scenario1.webp)
...
Well luckily virtual memory is enabled and so memory is divided in equal-sized 4-kilobyte chunks called pages (this scenario is theoretical and ignores the fact that the operating
system itself would also be taking up memory).

![Example scenario 1.1](/images/documents/virtual-memory-explained/scenario1-1.webp)
...
in a while, say the third page from Minecraft, store it on the main SSD, and place Chrome's page in the now empty space.

![Example scenario 1.2](/images/documents/virtual-memory-explained/scenario1-2.webp)
...
The last 12 bits of the virtual address (bits 0-11) represent the "offset" within the 4KB page, specifying the exact byte we want from those 4,096. So to recap a 48-bit
...
thing is: with a single table, you&apos;d need an entry for every possible page in the entire 48-bit address space. Each entry is 8 bytes so that would get massive
real fast.
