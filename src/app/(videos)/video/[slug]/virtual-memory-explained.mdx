...
arises: security and resource management. On one hand each application should be mostly isolated one from another. 
...
In the above illustration ssigned the appropriate index to each digit of the concerned binary number from right to left starting from 0. This is a very important step because
...
A single binary digit is called a bit. The number we were working with was of 4 bits. Bits are a unit of measurement used to quantify memory just like you use centimeters to quantify
...
I said I wouldn't delve into hexadecimal, but I'll at least tell you how it supports this many digits if we only know 9: It uses the alphabet. Here are all these possible digits: 0, 1,
...
The system can take the first two pages of Chrome, fit them where Discord was, and take the ramaining page and fit it under Minecraft. That's memory paging. 
